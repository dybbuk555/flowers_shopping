...
export const sendMail = async (fromEmail, values, replyEmail) => {
  try {
...
@media (prefers-color-scheme: dark) {
  ...
  <body class="dark-mode-bg-gray-999" style="margin: 0; width: 100%; padding: 0; word-break: break-word; -webkit-font-smoothing: antialiased; background-color: #f3f4f6;">
    ...
    <p style="margin: 0; margin-bottom: 24px;">
     Your order for ${values.firstName} ${values.lastName} has been received. We are working to deliver at ${values.address}, ${values.region}, ${values.city} on ${values.deliveryDate}.
     We will reach you on ${values.senderPhone} or ${values.email} if we have any questions.
    </p>
    <p style="margin: 0; margin-bottom: 24px;">
    Please take note of your tracking ID: ${values.trackingID}.
    </p>
...
`,
    ...
    return response ? { ok: true } : { response: false, msg: response };
  } catch (error) {
...
export const sendAdminAlert = async (toEmail, fromEmail, values) => {
  try {
    const response = await transporter.sendMail({
  ...
  @media (prefers-color-scheme: dark) {
    ...
    <body class="dark-mode-bg-gray-999" style="margin: 0; width: 100%; padding: 0; word-break: break-word; -webkit-font-smoothing: antialiased; background-color: #f3f4f6;">
      ...
      <p style="margin: 0; margin-bottom: 24px;">
       You have a new order by ${values.email}. Payment of ${values.amount} cedis received. Delivery date is ${values.deliveryDate}
      </p>
